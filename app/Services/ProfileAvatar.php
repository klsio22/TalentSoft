...
class ProfileAvatar
{
  ...
  /**
   * Verifica se o modelo tem um avatar válido
   *
   * @return bool true se o modelo tem um avatar válido
   */
  public function hasValidAvatar(): bool
  {
    if (empty($this->model->avatar_name)) {
      return false;
    }
    
    $avatarPath = $this->getAbsoluteSavedFilePath();
    return file_exists($avatarPath) && is_file($avatarPath);
  }
   ...
   *
   * @return bool true se o avatar foi removido com sucesso
   */
   ...
   *
   * @return string|null O último erro ou null se não houver erro
   */
